...
const dataLocations: ApiResponse<Location> = {
    info: {
        count: 126,
        pages: 7,
...
describe('Tests for AppRoutes', () => {
    ...
    test('should show home page when on init', async () => {
        ...
        await waitFor(() => expect(screen.getAllByLabelText('container-character').length).toBeGreaterThan(0))
        await waitFor(() => expect(screen.getByText('Rick Sanchez')).toBeInTheDocument())
    });

    test('should be active characters nav bar item', async () => {
        mock.onGet('https://rickandmortyapi.com/api/character?name=&page=1').replyOnce(200, data);
        await act(() =>
            renderWithClient(
                queryClient,
        ...
        await waitFor(() =>{
            const navItem = screen.getByLabelText('characters-navbar-item');
            const classesArray = Array.from(navItem.classList);
            expect(classesArray.includes('active')).toBeTruthy();
        })
    });

    test('should show locations page', async () => {
        mock.onGet('https://rickandmortyapi.com/api/location?name=&page=1').replyOnce(200, dataLocations);
        ...
        await waitFor(() => expect(container.getElementsByClassName('cardLocation').length).toBeGreaterThan(0))
        await waitFor(() =>{
            const navItem = screen.getByLabelText('locations-navbar-item');
            const classesArray = Array.from(navItem.classList);
            expect(classesArray.includes('active')).toBeTruthy();
        })
    });
