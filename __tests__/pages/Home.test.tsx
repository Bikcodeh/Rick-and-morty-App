...
import { getData, useGetData } from '../../src/hooks/useGetData';
...
describe('Tests for <Home />', () => {
    ...
    /*  test('should display loading when init ', () => {
         jest
             .spyOn(reactQuery, 'useQuery')
             .mockImplementation(
                 jest
                     .fn()
                     .mockReturnValue({ data: undefined, isLoading: true, isSuccess: false, isError: false })
             )
         render(
             <BrowserRouter>
     ...
     test('should render correctly', () => {
         jest
             ...
             .mockImplementation(
                 jest
                     .fn()
                     .mockReturnValue({ data: dataSuccess, isLoading: false, isSuccess: true, isError: false })
             )
         render(
             <BrowserRouter>
                 <QueryClientProvider client={client}>
     ...
     test('should paginate when click on next button correctly', async () => {
         ...
         jest
             ...
             .mockImplementation(
                 jest
                     .fn()
                     .mockReturnValue({ data: dataSuccess2, isLoading: false, isSuccess: true, isError: false })
             )
         fireEvent.click(await screen.findByLabelText('next-button'))
         expect(container.getElementsByClassName('characterContainer').length).toBe(6)
 
     }); */

    test('should paginate when click on prev button correctly', () => {
        ...
        //(rickAndMortyApi.get as jest.Mock).mockResolvedValue(mockedResponse)
        render(
            <BrowserRouter>
                <QueryClientProvider client={client}>
                    <Home />
        ...
        //fireEvent.click(await screen.findByLabelText('next-button'))
        //expect(screen.getByLabelText('prev-button')).toBeEnabled();

        // fireEvent.click(await screen.findByLabelText('prev-button'))
        screen.debug();
    });
});